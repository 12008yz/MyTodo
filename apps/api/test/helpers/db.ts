...
export async function truncateAuthTables(db: ReturnType<typeof createDb>["db"]): Promise<void> {
  await db.execute(
    sql`TRUNCATE TABLE english_progress, english_settings, english_lessons, daily_stats, goal_snapshots, doom_scroll_sessions, pomodoro_sessions, checkins, habits, refresh_tokens, users RESTART IDENTITY CASCADE`,
  );
}

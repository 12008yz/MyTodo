...
**Цель:** полный flow §4 после регистрации.

- [ ] Шаг профиля: вес, рост, сон, `free_time_min`, timezone (auto)
- [ ] Светлая сторона: выбор/создание привычек (до 6 суммарно позже; на онбординге — стартовый набор)
- [ ] Калибровка baseline для каждой привычки
- [ ] Тёмная сторона: выбор вредных привычек + baseline
- [ ] `PATCH /me` → `onboarding_completed=true`
- [ ] Жёсткость уведомлений (harshness 1–3)

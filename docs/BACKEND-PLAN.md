...
- [ ] **Прогрессия `current_goal`** — после снимка, для следующего дня:
...
- После закрытия success светлой — `current_goal` вырос; в `goal_snapshots` за этот день — **старая** цель
...
- [ ] `.env.example`
- [ ] Интеграционные тесты: auth, checkins, day-close, progression, `goal_snapshots` + `daily_stats`, billing webhook, pledge, push dedup
...
- [ ] Worker: закрытие дня, прогрессия, `goal_snapshots`, `daily_stats`, doom-expiry, pledge expiry, push с дедупом

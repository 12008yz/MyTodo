...
2. Внутри блока: **БД → shared (Zod) → сервисы → API → тесты**.
...
### Структура кода (ТЗ §16.4)

```
apps/api/           # Fastify, роуты, worker (BullMQ)
packages/shared/    # Zod, типы, константы
...
| Превью «цель завтра» | В API-ответах | Считается на лету, **не** пишется в `habits.current_goal` до закрытия дня |
...
- [ ] Turborepo: `apps/api`, `packages/shared`, `docker-compose.yml`
- [ ] Postgres 16 + Redis
...
- [ ] Vitest + supertest: smoke-тест health

### Критерии готовности

- `pnpm dev` поднимает API
- `docker compose up` — Postgres + Redis
- `pnpm test` — health green
...
- [ ] `POST /checkins` — `{ habit_id, date?, value?, status? }`
  ...
  - **пропуск:** `status: 'skipped'` — только светлая; ≤2 раз пн–вс на привычку
  - при активном залоге на привычку: `skipped` → `400` (§3.6)
- [ ] В ответе: `preview_next_goal` (вычисляемое, не сохраняем)
...
- Batch + конфликт 409
- Тест: после success чекина `current_goal` тот же; после worker (блок 9) — изменился
...
- `preview_next_goal` совпадает с тем, что применит worker
...
- [ ] `GET /stats/week` — 7 дней, статус 🟢🟡🔴⚪ (§10.2)
- [ ] `GET /stats/calendar?month=YYYY-MM` — сетка + детали дня
- [ ] `GET /stats/month` — % успеха, срывы по привычкам
- [ ] `GET /stats/habits/:id/progress?period=week|month|quarter` — точки для Recharts (§27)
- [ ] `GET /stats/summary` — **v1.0:** тепловая карта месяца; **v1.1:** + круговая диаграмма (§27.6)
- [ ] История `current_goal`: расчёт из чекинов + снимки при закрытии дня (опционально таблица `goal_snapshots`)
...
- Неделя и месяц согласованы с чекинами
- Progress: 30 точек для графика
...
**Цель:** уроки, прогресс, пропуски — **до** worker, чтобы тот закрывал уроки в 23:59.

### Зависимости

Блок 2, 4 (переиспользовать счётчик пропусков пн–вс).
...
- [ ] `complete`: `watched_sec` ≥ 80% `duration_sec` → `success`; **завтра** `current_day + 1` (при закрытии дня или сразу — зафиксировать в сервисе; рекомендуем **при complete** менять `current_day` на следующий урок, а `english_progress` за день = success)
- [ ] `skip` — ≤2/нед; **не** увеличивает `current_day`
- [ ] Не влияет на `daily_budget_min` (§2.3)
- [ ] Если `is_enabled = false` — роуты возвращают `404` или `{ enabled: false }`

### Критерии готовности

- Цепочка уроков 1→2→3 при complete
- 3-й skip → `400`
- Skip не продвигает урок
...
- [ ] **Автозакрытие дня** (23:59 local): для каждой активной привычки:
...
- [ ] **Прогрессия `current_goal`** — только здесь, после финального статуса дня:
...
- [ ] **Английский:** если `is_enabled` и нет success/skip за день → `english_progress.fail`; урок **не** продвигается
- [ ] **Doom-scroll:** истёкшие сессии → `completed`, минуты в чекин
- [ ] Идемпотентность: повторный прогон не дублирует закрытие
- [ ] Лог `day_closed`
...
| Блок | Задача worker |
| ---- | ------------- |
| 11 | Истечение 30-дневного залога → success/failed + refund |
...
- После закрытия success светлой — `current_goal` вырос
- English fail при неотметке
- Повторный cron — без дублей
- Тест timezone: Europe/Moscow и Asia/Vladivostok
...
- [ ] Чекин `fail` по привычке с активным залогом → pledge `failed`
- [ ] Worker (дополнение блока 9): `started_at + 30 дней`, все дни success → `success`, refund, badge `steel_character`
...
- Fail чекина → pledge `failed`
- 30 дней без fail → `success` + refund
...
- [ ] Миграции: `push_subscriptions`, `notification_templates`
...
- [ ] Worker (дополнение блока 9):
  ...
  - cheer 3–5/день для курения `abstinence` без срыва сегодня (§9.2)
  - doom-scroll: job +15 мин → `doom_scroll_end`
  - лимит 30 мин → `doom_scroll_limit`
- [ ] Не слать, если `silence_mode_until > now`
- [ ] Не слать cheer после срыва сегодня
- [ ] Логи `push_sent` / `push_failed`
...
- [ ] `.env.example`
- [ ] Интеграционные тесты: auth, checkins, day-close, progression, billing webhook, pledge
...
| 2 | `habits` | 3 |
| 3 | `checkins` | 4 |
| 4 | `pomodoro_sessions`, `doom_scroll_sessions` | 6 |
| 5 | `english_lessons`, `english_settings`, `english_progress` | 8 |
| 6 | `subscriptions` | 10 |
| 7 | `pledges`, `user_badges` | 11 |
| 8 | `push_subscriptions`, `notification_templates` | 13 |
...
- [ ] Worker: закрытие дня, прогрессия, doom-expiry, pledge expiry, push-расписание
...
| Прогрессия goal | Размыто | Только при закрытии дня (worker) |

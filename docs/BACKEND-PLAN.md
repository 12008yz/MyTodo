...
**Статус:** ✅ **Backend v1.0 завершён** (блоки 1–14, 20.06.2026) · Frontend — [FRONTEND-PLAN.md](./FRONTEND-PLAN.md)  
**Подход:** сначала весь backend, потом frontend  
**Связанный документ:** [TZ.md](./TZ.md)  
**Дата:** 19.06.2026 (ревизия 5)
...
| 15 | Backend v1.1 (опционально) | 5–7 дней | §23, §27.6 | ⏳ после frontend MVP |
...
- [x] Turborepo: `apps/api`, `packages/shared`, **`packages/domain`**, `docker-compose.yml`
- [x] Postgres 16 + Redis
- [x] Fastify 5 + TypeScript: CORS, helmet, rate-limit
- [x] Drizzle: миграции, `db:migrate`, `db:studio`
- [x] Pino, глобальный error handler, Sentry (заглушка в dev)
- [x] `GET /api/v1/health` → `{ status, db, redis }`
- [x] `packages/shared`: `ApiError`, коды HTTP
- [x] **`packages/domain`:** scaffold пакета, Vitest (тесты без БД/Redis), экспорт пустого `index`
- [x] Vitest + supertest: smoke-тест health; **`pnpm --filter @mytodo/domain test`** — green (пустой suite ок)
...
- [x] Миграция `users`, `refresh_tokens`
- [x] **Отличие от SQL в ТЗ §17:** поля онбординга (`weight_kg`, `height_cm`, `wake_time`, `sleep_time`, `free_time_min`) — **nullable** до завершения онбординга; флаг `onboarding_completed BOOLEAN DEFAULT false`
- [x] Поля сразу в `users`: `silence_mode_until`, `silence_mode_used_at` (логика в блоке 12)
- [x] `POST /auth/register` — email, пароль, имя, возраст, пол; `trial_ends_at = now + 3 дня`
- [x] `POST /auth/login`, `/auth/refresh`, `/auth/logout`
- [x] Middleware `authenticate`
- [x] `GET /me`, `PATCH /me`
- [x] `PATCH /me` (онбординг): вес, рост, режим сна, `free_time_min`, `timezone`, помодоро, `harshness_level` → `onboarding_completed = true`
- [x] `daily_budget_min = min(free_time_min, 60)` при сохранении профиля (§2.3)
- [x] Timezone: авто из клиента или `Europe/Moscow` (§2.2)
- [x] Shared: `TRIAL_DAYS = 3`, `SUBSCRIPTION_PLANS`, `PLEDGE_AMOUNT = 5000`
...
- [x] `allows_weekly_skip = true` только для `side = light`
- [x] `PATCH /habits/:id` — ручное изменение цели (§3.4)
- [x] `harshness_level` при создании привычки берётся из профиля; смена только для новых (§11)
...
- [x] Миграция `checkins`
- [x] `GET /checkins?date=YYYY-MM-DD`
- [x] `POST /checkins` — `{ habit_id, date?, value?, status? }`; статус через **`domain.resolveCheckinStatus`**
...
- [x] В ответе: `preview_next_goal` = **`domain.computeNextGoal(habit, dayStatus)`** (не дублировать формулу в сервисе)
- [x] **Не менять** `habits.current_goal` в этом блоке
- [x] `POST /checkins/batch` — офлайн; конфликт `updated_at` → `409` + id конфликтов
- [x] Уникальность `(habit_id, date)`
- [x] Middleware `requireAccess` — заглушка (всегда ok) до блока 10
...
- [x] `GET /today/light` — привычки, `current_goal`, чекины сегодня, бюджет минут, `preview_next_goal`, серия
- [x] `GET /today/dark` — лимиты, таймеры, doom-scroll active, статусы
- [x] `GET /habits/:id/timer` — от `last_relapse_at`, timezone-aware
- [x] Данные для 4 карточек статистики (§6.1): выполнено сегодня, срывы за неделю, минуты/🍅, серия
...
- [x] Миграции: `pomodoro_sessions`, `doom_scroll_sessions`
- [x] Помодоро: `start` / `complete` / `stop` / `active` — одна активная сессия на привычку
- [x] `complete` → добавляет минуты в чекин дня (пересчёт value + status)
- [x] Doom-scroll:
...
- [x] Истечение 15 мин без stop — завершение сессии в worker (блок 9), минуты засчитываются
...
- [x] `GET /stats/week` — читать из `daily_stats` (агрегат по дням); не считать тяжёлые join на лету
- [x] `GET /stats/calendar?month=YYYY-MM` — цвет дня из `daily_stats` + детали
- [x] `GET /stats/month` — % успеха, срывы из `daily_stats`
- [x] `GET /stats/habits/:id/progress?period=week|month|quarter` — `goal_snapshots` (линия цели) + `checkins` / `daily_stats` (столбцы value)
- [x] `GET /stats/summary` — **v1.0:** тепловая карта из `daily_stats`; **v1.1:** + круговая диаграмма (§27.6)
...
- [x] Миграции: `english_lessons`, `english_settings`, `english_progress`
- [x] Минимальный fixture: 3–5 уроков в тестах (полный seed — блок 14)
- [x] `GET /english/today`, `POST /english/complete`, `POST /english/skip`, `GET /english/history`, `PATCH /english/settings`
- [x] `complete`: `watched_sec` ≥ 80% `duration_sec` → `english_progress` за сегодня = `success`; **`current_day` не менять**
- [x] В ответе `complete` / `today`: `preview_next_day` = **`domain.computeNextEnglishDay(current_day, dayStatus)`**
- [x] `skip` — ≤2/нед (`domain.canSkipThisWeek`); `english_progress` = `skipped`; **`current_day` не менять**
- [x] Не влияет на `daily_budget_min` (§2.3)
- [x] Если `is_enabled = false` — роуты возвращают `404` или `{ enabled: false }`
- [x] Unit-тесты `computeNextEnglishDay`, `closeEnglishDay` в domain
...
- [x] BullMQ + Redis, команда `pnpm worker`
- [x] **Автозакрытие дня** (23:59 local) — оркестрация в worker, правила в **`domain.closeDayForHabit`** / **`domain.closeEnglishDay`**:
...
- [x] **`goal_snapshots`:** для каждой привычки за день `D` → сохранить `goal_value = current_goal` (цель, по которой оценивался день)
- [x] **`daily_stats`:** итог дня → `status`, `value`, `minutes_total`
- [x] **Прогрессия `current_goal`** — **`domain.computeNextGoal`** (та же функция, что `preview_next_goal` в API):
...
- [x] **Английский:** `domain.closeEnglishDay` — нет записи за день → `fail`; при `success` за день → **`current_day + 1`** через `domain.computeNextEnglishDay`; `fail` / `skipped` → `current_day` без изменений
- [x] **Doom-scroll:** истёкшие сессии → `completed`, минуты в чекин
- [x] Идемпотентность: повторный прогон не дублирует закрытие, снимки и `daily_stats` (UPSERT / проверка существования)
- [x] Лог `day_closed`
...
- [x] Миграция `subscriptions`
- [x] `POST /billing/subscribe` — `{ plan: 'monthly' | '2months' | '3months' }`
- [x] Цены: 1990 / 3790 / 5490 ₽; периоды: 30 / 60 / 90 дней
- [x] `monthly` — рекуррент; `2months` / `3months` — разовая оплата, без renewal
- [x] `POST /billing/cancel` — только `monthly`; доступ до `current_period_end`
- [x] `POST /billing/webhook` — подпись ЮKassa, идемпотентность
- [x] `past_due`: retry через 3 дня, макс. 3 попытки → `expired` (§24.2)
- [x] Middleware `requireAccess` (заменить заглушку блока 4):
...
- [x] Исключения из middleware: `/auth/*`, `/health`, `/billing/webhook`
...
- [x] Миграции: `pledges`, `user_badges`
- [x] `GET /pledges`, `POST /pledges` — `{ habit_id, charity_fund }`
- [x] `charity_fund`: `oncology` | `children` | `animals` (§8.4)
- [x] Проверки: 1 активный залог / месяц; привычка принадлежит user
- [x] Оплата 5000 ₽ через ЮKassa при активации
- [x] Чекин `fail` по привычке с активным залогом → **сразу** `pledges.status = failed` (не ждать 30 дней)
- [x] Worker (дополнение блока 9) при `started_at + 30 дней`:
...
- [x] `refund_error` → Sentry (§24.2)
- [x] Admin manual close — блок 14
...
- [x] `GET /me/export` — ZIP: `profile.json`, `habits.csv`, `checkins.csv`, `english_progress.csv` (§11.1)
- [x] `DELETE /me` — при активном залоге: pledge → `failed`, затем каскадное удаление (§24.4)
- [x] `PATCH /me` — `{ enable_silence_mode: true }`:
  - `silence_mode_until = now + 24h`
  - не чаще 1 раза в 30 дней (`silence_mode_used_at`)
  - жёсткость → мягкая на 24ч (в API ответа)
- [x] Смена `timezone` — push-расписание с **следующего** дня; чекины не пересчитываются (§24.7)
...
- [x] Миграции: `push_subscriptions`, `notification_templates`, **`push_delivery_log`**
- [x] Минимальный seed шаблонов в тестах (полный — блок 14)
- [x] `POST /push/subscribe`, `DELETE /push/subscribe`, `POST /push/test`
- [x] VAPID в env
- [x] **Мгновенные** (в API, не cron): `relapse`, `success` при чекине (§9.5)
- [x] **При doom-scroll start:** push `doom_scroll_start` (§9.3)
- [x] **Дедупликация расписания (обязательно):**
...
- [x] Не слать, если `silence_mode_until > now`
- [x] Не слать cheer после срыва сегодня
- [x] Логи `push_sent` / `push_failed` (Pino); запись в `push_delivery_log` **до** отправки (at-least-once safe)
...
- [x] Admin API `/api/v1/admin/*` (§25.3)
- [x] `pnpm seed` (идемпотентный): 30 уроков, все шаблоны push, users (user, trial, admin)
- [x] `docs/API.md` или OpenAPI
- [x] `.env.example`
- [x] Интеграционные тесты: auth, checkins, day-close, progression, **`preview_next_goal` = post-worker goal**, `goal_snapshots` + `daily_stats`, billing webhook, pledge, push dedup, **english day advance only after worker**
- [x] Чеклист backend-части §19
...
- [x] Все эндпоинты §18 + `POST .../doom-scroll/stop`
- [x] Worker: закрытие дня через **`packages/domain`**; прогрессия, `goal_snapshots`, `daily_stats`, doom-expiry, pledge expiry, push с дедупом
- [x] Trial 3 дня; тарифы 1990 / 3790 / 5490
- [x] Залог 5000 ₽ + refund
- [ ] ЮKassa staging (код готов — нужны ключи в env на staging/prod)
- [x] Seed + admin API
- [x] Критичные тесты green (218: 118 API + 95 domain + 5 shared)
- [x] `.env.example`
